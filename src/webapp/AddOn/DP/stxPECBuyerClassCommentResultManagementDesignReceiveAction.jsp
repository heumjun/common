...
<%
   ...
   // String loginID = SessionUtils.getUserId();
	...
	String sProcessFlag = StringUtil.setEmptyExt(emxGetParameter(request, "processFlag"));
	String sProcessPerson = StringUtil.setEmptyExt(emxGetParameter(request, "processPerson"));
	String sRevNoDeptList = URLDecoder.decode(StringUtil.setEmptyExt(emxGetParameter(request, "revNoDeptList")),"UTF-8");
	String sCommentMsg = URLDecoder.decode(StringUtil.setEmptyExt(emxGetParameter(request, "commentMsg")),"UTF-8");
    ...
    try 
	{
		...
		else if("3".equals(sProcessFlag))
		{
			...
			// 기술기획 수신문서 접수 담당자(정은지)에게 반송 메일 전송
			ArrayList toList = new ArrayList();

			String toUser = "eun727@onestx.com";  // 기술기획 수신문서 upload 담당자 (정은지)
