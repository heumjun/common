<%--  
§DESCRIPTION: 기자재 발주 관리 및 DP일정 통합관리 등록 - PR 발행
...
<%
	...
	System.out.println( "~~ CreatePR start :: stxPECEquipItemPurchasingScheduleManagementRegisterCreatePR" );
    // 파일 업로드 디렉토리 설정
	...
	******/
    ...
    //System.out.println("~~ CreatePR sSelCheckbox = "+sSelCheckbox); 
    ...
    if(!"".equals(sSelCheckbox))
    {
        ...
        while(st.hasMoreTokens())
        {
            ...
            resultMap.put("drawingTitle", multi.getParameter("drawingTitle"+checkboxValue) == null ? "" : multi.getParameter("drawingTitle"+checkboxValue));
            resultMap.put("drDate", multi.getParameter("drDate"+checkboxValue) == null ? "" : multi.getParameter("drDate"+checkboxValue));
            resultMap.put("makerList", multi.getParameter("makerList"+checkboxValue) == null ? "" : multi.getParameter("makerList"+checkboxValue));
            resultMap.put("itemCode", multi.getParameter("itemCode"+checkboxValue) == null ? "" : multi.getParameter("itemCode"+checkboxValue));
            resultMap.put("fileName", multi.getFilesystemName("fileName"+checkboxValue) == null ? "" : multi.getFilesystemName("fileName"+checkboxValue));
            resultMap.put("fileType", multi.getContentType("fileName"+checkboxValue) == null ? "" : multi.getContentType("fileName"+checkboxValue));
            resultMap.put("pndDate", multi.getParameter("pndDate"+checkboxValue) == null ? "" : multi.getParameter("pndDate"+checkboxValue));
            resultMap.put("pr_dept_changeable_yn", multi.getParameter("pr_dept_changeable_yn"+checkboxValue) == null ? "" : multi.getParameter("pr_dept_changeable_yn"+checkboxValue));
            resultMap.put("pr_dept_changeable_owner", multi.getParameter("pr_dept_changeable_owner"+checkboxValue) == null ? "" : multi.getParameter("pr_dept_changeable_owner"+checkboxValue));
    ...
    if(resultList.size() > 0)
    {
        ...
        try
        {
            ...
            for(int j=0; j < resultList.size(); j++)
            {
                ...
                String makerList =  (String) resultMap.get("makerList");
                ...
                String pr_dept_changeable_yn =  (String) resultMap.get("pr_dept_changeable_yn");
                String pr_dept_changeable_owner =  (String) resultMap.get("pr_dept_changeable_owner");

                //System.out.println(" CreatePR start pr == "+drawingNo+"   :  "+fileName);



                // 견적기획 발행 도면의 경우는 선택된 사용자로, 그 외는 접속한 사번으로 pr 발행
                String prCreator = "";
                if("Y".equals(pr_dept_changeable_yn))
                {
                    prCreator = pr_dept_changeable_owner;      
                } else {
                ...
                {                    
                    ...
                    // 2011-05-06 Kang seonjung : 이미 PR DATA가 있을 경우는 Continue.. 중복 실행 방지?
                    StringBuffer preCheckSQL = new StringBuffer();
                    preCheckSQL.append("SELECT *                                                            \n");
                    preCheckSQL.append("  FROM STX_PO_EQUIPMENT_DP                                          \n");
                    preCheckSQL.append(" WHERE PROJECT_NUMBER = '"+projectNo+"'                             \n");
                    preCheckSQL.append("   AND DRAWING_NO = '"+drawingNo+"'                                 \n");
                    preCheckSQL.append("   AND ORIGINAL_PR_FLAG = 'Y'                                       \n");
                      
                    rset = stmt.executeQuery(preCheckSQL.toString());

                    if(rset.next())
                    {
                        System.out.println(" preCheckSQL  data exist !!!! continue ");
                        continue;                        
                    }
				   ...
				   // pndDateSQL.append("SELECT TO_CHAR(DECODE(EQUIP_DATE_1ST, NULL, DECODE(JOB_FIRST_NEED, NULL, FIRST_NEED_DATE, JOB_FIRST_NEED), EQUIP_DATE_1ST), 'YYYY-MM-DD') AS PND_DATE   ");
					...
					if("".equals(pndDate_ERP))
					{
						...
						connDP = DBConnect.getDBConnection("SDPS");
               ...
               // stmt3 = conn.createStatement();
                ...
                // 2011-05-06 Kang seonjung : 이미 PR DATA가 있을 경우는 Continue.. 중복 실행 방지?
                StringBuffer preCheckSQL1 = new StringBuffer();
                preCheckSQL1.append("SELECT *                                                            \n");
                preCheckSQL1.append("  FROM STX_PO_EQUIPMENT_DP                                          \n");
                preCheckSQL1.append(" WHERE PROJECT_NUMBER = '"+projectNo+"'                             \n");
                preCheckSQL1.append("   AND DRAWING_NO = '"+drawingNo+"'                                 \n");
                preCheckSQL1.append("   AND ORIGINAL_PR_FLAG = 'Y'                                       \n");
                
                rset1 = stmt1.executeQuery(preCheckSQL1.toString());

                if(rset1.next())
                {
                    System.out.println(" preCheckSQL1  data exist !!!! continue ");
                    continue;                        
                }
               ...
               // dpInsertSQL.append(" 									APPROVAL_SEND_DATE,  \n");
                ...
                pstmt7.setString(4, makerList); 
                pstmt7.setString(5, deptId);
                pstmt7.setString(6, drawingNo);
                pstmt7.setString(7, retPrNo);
...
/*    var urlStr = "stxPECEquipItemPurchasingScheduleManagementRegisterMain.jsp?projectNo=" + parent.EQUIP_ITEM_PURCHASING_MANAGEMENT_HEADER.frmEquipItemPurchasingManagementHeader.projectList.value;
...
*/
    ...
    parent.EQUIP_ITEM_PURCHASING_MANAGEMENT_HEADER.frmEquipItemPurchasingManagementHeader.createPRrunning.value = "FALSE";
    alert("<%=resultMsg%>");
