...
<%@page import="com.stxdis.util.util.SessionUtils"%>
...
<script language="javascript">
    ...
    function showProjectSelectWin() 
    {
        ...
        var selectedProjects = window.showModalDialog("stxPECDPInput_ProjectSelect.jsp?" + paramStr, "", sProperties);
    ...
    function showDesignHoursViewWin() 
    {
        ...
        var urlStr = "stxPECDPInput_InputListViewFS.jsp?loginID=" + DPInputHeader.loginID.value;
    ...
    function showDesignHoursApprViewWin() 
    {
        ...
        var dhApprCheckResult = window.showModalDialog("stxPECDPInput_ApprovalsViewFS.jsp", paramObj, sProperties);
    }
    ...
    function viewDPInputs(showMsg, dateSelected, workingDayYN, MHConfirmYN, workdaysGap)
    {
        ...
        // 선택된 날짜의 시수입력 사항을 조회
        var urlStr = "stxPECDPInputMain.jsp?deptCode=" + ((DPInputHeader.departmentSel.value).split("|"))[0];
        urlStr += "&deptName=" + ((DPInputHeader.departmentSel.value).split("|"))[1];
        urlStr += "&designerID=" + ((DPInputHeader.dpDesignerIDSel.value).split("|"))[0];
        urlStr += "&designerName=" + ((DPInputHeader.dpDesignerIDSel.value).split("|"))[1];
        urlStr += "&dateSelected=" + dateSelected;
        urlStr += "&workingDayYN=" + workingDayYN;
        urlStr += "&MHConfirmYN=" + MHConfirmYN;
        urlStr += "&workdaysGap=" + workdaysGap;
        urlStr += "&loginID=" + DPInputHeader.loginID.value;
        urlStr += "&showMsg=" + showMsg;
    ...
    function departmentSelChanged() 
    {
		...
		xmlHttp.open("GET", "stxPECDP_GeneralAjaxProcess.jsp?requestProc=GetPartPersons&departCode=" + ((DPInputHeader.departmentSel.value).split("|"))[0], false);
		xmlHttp.send(null);
    ...
    function dpDesignerSelChanged() 
    {
        if (DPInputHeader.dpDesignerIDSel.value == "") {
            ...
            xmlHttp.open("GET", "stxPECDP_GeneralAjaxProcess.jsp?requestProc=GetConfirmYN&dpDesignerID=" + ((DPInputHeader.dpDesignerIDSel.value).split("|"))[0] + 
    ...
    function showDPInputLockWin()
    {
        ...
        var result = window.showModalDialog("stxPECDPInput_LockControl.jsp", "", sProperties);
    }
    ...
    dateChanged();
    parent.DP_TIMESELECT.location = "stxPECDPInputTimeSelect.jsp"; // Header 페이지가 먼저 로드된 후 Time Select 창이 로드되도록.
    viewDPInputs("false");
