...
§FILENAME: stxPECEquipItemPurchasingScheduleManagementRegisterSearchPrPoInfo.jsp
§CHANGING HISTORY: 
...
<%!
	...
	private String getDrawingInfo(String projectNo, String prNo, String drawingNo) throws Exception
	{
        ...
        try {
			...
			/*** 2013-06-13 : PR Info Header를 PR Description으로 변경
			...
			***/
            ...
            drawingInfoQuery.append("SELECT DESCRIPTION                                  \n"); 
            drawingInfoQuery.append("  FROM PO_REQUISITION_HEADERS_ALL PRHA              \n"); 
            drawingInfoQuery.append(" WHERE PRHA.SEGMENT1 = '"+prNo+"'                   \n");
			...
			while (rset.next())
            {
                ...
                returnString = "[  "+drawingNo+" - "+rset.getString(1)+"  ]";
            }
        } catch( Exception ex ) {
            ex.printStackTrace();
        }finally{
	...
	// 추가예산신청 대상 PR 인지 확인
	private boolean searchAddCharge(String prNo) throws Exception
	{
        if (StringUtil.isNullString(prNo)) throw new Exception("Pr No. is null");
        ...
        try {
            ...
            prStateQuery.append("SELECT *                                      \n"); 
            prStateQuery.append("  FROM STX_PO_EQUIPMENT_DP_CHARGE             \n"); 
            prStateQuery.append(" WHERE 1=1                                    \n");
            prStateQuery.append("   AND PR_NO = '"+prNo+"'                     \n");
...
<%
    ...
    try {
		...
		addChargeFlag = searchAddCharge(prNo);
...
<script language="javascript">
    ...
    function approvePR()
    {
        ...
        if(confirm("승인요청 하시겠습니까? 결과가 나올때까지 잠시만 기다려주십시오."))
        {
            ...
            frmSearchPurchasingCode.action = "stxPECEquipItemPurchasingScheduleManagementRegisterApprovePR.jsp";
    ...
    function deletePR()
    {
        ...
        {
            var isDone = true;
            frmSearchPurchasingCode.action = "stxPECEquipItemPurchasingScheduleManagementRegisterDeletePR.jsp";
            frmSearchPurchasingCode.submit();    
        }
    }
</script>
...
<hr>   
    ...
    %>
        <tr height="45">
            <td width="750" style="text-align:left;">
                <table cellspacing="1" cellpadding="0">
                    <tr height="25">
                        ...
                        %>
							<td width="30">&nbsp;</td>
							<td class="td_standardBold" style="background-color:#FFE400;" width="120">추가 예산 대상							
							</td>
                        <%
                        }
                        %>
