<%--  
§DESCRIPTION: 도면 출도 건 수 조회 Header Toolbar 부분
§AUTHOR (MODIFIER): Hyesoo Kim
§FILENAME: stxPECDPHardCopyDwgDeployedCountHeader.jsp
§CHANGING HISTORY: 
...
<%--========================== HTML HEAD ===================================--%>
<html>
<head><title>도면 출도 건 수 조회</title></head>
...
<%--========================== HTML BODY ===================================--%>
<body>
<form name="DPHardCopyDwgDeployedCntHeader">
    ...
    <table width="100%" cellSpacing="0" border="1" align="left">
        ...
        <tr height="30">
            ...
            <td width="320">기간                
                ...
                <a href="javascript:showCalendar('DPHardCopyDwgDeployedCntHeader', 'dateSelectedFrom', '', false, dateChanged);">
                ...
                <a href="javascript:showCalendar('DPHardCopyDwgDeployedCntHeader', 'dateSelectedTo', '', false, dateChanged);">
                    <img src="../common/images/iconSmallCalendar.gif" border="0" valign="absmiddle"></a>
                <!--
            ...
            <td>
                <input type="button" name="ViewButton" value='조 회' class="button_simple" onclick="viewHardCopyDeployedCntData();"/>
            </td>
        </tr>
...
<script language="javascript">
    ...
    parent.HARDCOPY_DEPLOYEDCNT_MAIN.location = "stxPECDPHardCopyDwgDeployedCountMain.jsp?showMsg=false&firstLoad=true"; 
    ...
    function showProjectSelectWin() 
    {
        ...
        var paramStr = "selectedProjects=" + DPHardCopyDwgDeployedCntHeader.projectList.value;
        paramStr += "&loginID=" + DPHardCopyDwgDeployedCntHeader.loginID.value;
        ...
        DPHardCopyDwgDeployedCntHeader.projectList.value = selectedProjects;
    }

    // 날짜 출력 문자열을 형식화
    function dateChanged()
    {
        var tmpStr = DPHardCopyDwgDeployedCntHeader.dateSelectedFrom.value;
        if (tmpStr != null || tmpStr.trim() != "") {
            var dateStr = formatDateStr(tmpStr);
            DPHardCopyDwgDeployedCntHeader.dateSelectedFrom.value = dateStr;
        }
        tmpStr = DPHardCopyDwgDeployedCntHeader.dateSelectedTo.value;
        if (tmpStr != null || tmpStr.trim() != "") {
            var dateStr = formatDateStr(tmpStr);
            DPHardCopyDwgDeployedCntHeader.dateSelectedTo.value = dateStr;
        }
    }

    // 조회
    function viewHardCopyDeployedCntData()
    {
        // 조회기간이 한 달 이내(Series 포함인 경우 1주일 이내)인지 체크
        var sDateFrom = DPHardCopyDwgDeployedCntHeader.dateSelectedFrom.value;
        var sDateTo = DPHardCopyDwgDeployedCntHeader.dateSelectedTo.value;
        ...
        if (!DPHardCopyDwgDeployedCntHeader.chkboxIncludeSeries.checked && iDayDiff >= 31) {
            alert("조회기간은 한 달(31일)을 초과할 수 없습니다!");
            return;
        }
        else if (DPHardCopyDwgDeployedCntHeader.chkboxIncludeSeries.checked && iDayDiff >= 7) {
            alert("Series 포함인 경우, 조회기간은 일주일(7일)을 초과할 수 없습니다!");
            return;
        }

        // 조회
        var urlStr = "stxPECDPHardCopyDwgDeployedCountMain.jsp?projectList=" + DPHardCopyDwgDeployedCntHeader.projectList.value;
        urlStr += "&deptCode=" + DPHardCopyDwgDeployedCntHeader.departmentList.value;
        urlStr += "&dateFrom=" + DPHardCopyDwgDeployedCntHeader.dateSelectedFrom.value;
        urlStr += "&dateTo=" + DPHardCopyDwgDeployedCntHeader.dateSelectedTo.value;
        urlStr += "&includeSeries=" + (DPHardCopyDwgDeployedCntHeader.chkboxIncludeSeries.checked ? "true" : "false");
        urlStr += "&includeEarlyRev=" + (DPHardCopyDwgDeployedCntHeader.chkboxIncludeEarlyRev.checked ? "true" : "false");
        urlStr += "&isAdmin=" + DPHardCopyDwgDeployedCntHeader.isAdmin.value;
        urlStr += "&showMsg=true&firstLoad=false";

        parent.HARDCOPY_DEPLOYEDCNT_MAIN.location = urlStr;
    }
    ...
    DPHardCopyDwgDeployedCntHeader.dateSelectedFrom.value = formatDateStr(fromYMD);
    DPHardCopyDwgDeployedCntHeader.dateSelectedTo.value = formatDateStr(toYMD);
