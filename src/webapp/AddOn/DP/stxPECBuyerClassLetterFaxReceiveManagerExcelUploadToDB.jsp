...
<%@ page import="stxship.dis.common.util.DisEGDecrypt" %>
<%@ page import="javax.activation.MimetypesFileTypeMap" %>
...
<%
	...
	String mimeType = mimeTypesMap.getContentType(fileObj1);
	String descryptFlag = DisEGDecrypt.buyerClassDecrypt(fileObj1, sTempDir, fileObj1.getName(), mimeType);
	
	if(!"N".equals(descryptFlag)) {
		String filename = fileObj1.getName();
		String fileExt = filename.substring(filename.lastIndexOf('.'));
		String fileNm = filename.substring(0, filename.lastIndexOf('.'));
		
		java.io.File fileObj2 = new java.io.File(sTempDir + fileNm + "_Decrypt" + fileExt);
		
		//원본파일 삭제
		//DisEGDecrypt.deleteDecryptFile(fileObj1);
		sDataFile = sTempDir + java.io.File.separator + fileObj2.getName();
	}
	...
	try{
		...
		try {
				...
				// 엑셀 파일의 전체 시트 중에 DATA가 있는 SHEET 대해서 체크(첫번째 시트로 인지 못하는 경우가 있는 듯)
				int sheetCount = excelLoader.getSheetCount();
				for(int k=0; k <= sheetCount; k++)
				{
					int tempRowCount = excelLoader.getDataRowCount(k);
					if(tempRowCount > -1)
					{
						i = k;
						break;
		...
		//System.out.println(ArrayList);
			...
			//복호화 파일 삭제
			//DisEGDecrypt.deleteDecryptFile(fileObj2);
