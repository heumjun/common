...
§FILENAME: stxPECDPInputMain.jsp
§CHANGING HISTORY: 
    ...
    %>
        <table width="100%" cellSpacing="1" cellpadding="0" border="0" align="left" bgcolor="#cccccc">
            <tr height="20" bgcolor="#e5e5e5">
                ...
                <td width="65" class="td_standardBold">공사번호*</td>
                ...
                <td width="295" class="td_standardBold">업무내용</td>
            ...
            for (int i = 0; i < timeKeys.length; i++)
            {
                ...
                for (int j = 0; j < mhInputList.size(); j++) {
                    ...
                    if (timeStr.equals((String)mhInputMap.get("START_TIME"))) {
                        ...
                        if (!projectNoData.equals("S000") && !projectNoData.equals("V0000") && !dwgCodeData.equals("*****") 
                            && !dwgCodeData.equals("#####") && !opCodeData.equals("45") && !opCodeData.equals("95") && event1Data.equals("")) 
                        {
                            event1Data = "(선택안함)";
                        }
                    }
                ...
                %>
                    <tr id="time<%=timeKey%>" height="20" bgcolor="#ffffff" <%if (i != 0 && sTimeData.equals("")) {%>style="display:none;"<%}%> OnMouseOver="trOnMouseOver(this, '<%=timeKey%>');" OnMouseOut="trOnMouseOut(this, '<%=timeKey%>');"  onclick="updateDrawingInfo('<%=timeKey%>');">
                        ...
                        <td class="td_standard">
                            <input name="pjt<%=timeKey%>" value="<%=projectNoData%>" class="input_noBorder" style="width:65px;text-align:center;" readonly onClick="showElementSel('pjt', '<%=timeKey%>');" />
                            <select name="pjtSel<%=timeKey%>" class="input_small" style="width:65px;display:none;background-color:#fff0f5;" onChange="projectSelChanged('pjt', '<%=timeKey%>');projectSelChangedAfter('<%=timeKey%>');">
                                <option value="">&nbsp;</option>
                        ...
                        <td class="td_standard">
                            ...
                            <select name="drwNoSel<%=timeKey%>" class="input_small" style="width:500px;display:none;background-color:#fff0f5;" onChange="drwNoSelChanged('drwNo', '<%=timeKey%>');">
                                <option value="" text="">&nbsp;</option>
                            </select>
                        </td>
                        <!-- OP CODE -->
                        <td class="td_standard" onClick="showOpBtn('<%=timeKey%>');">
                            <input name="op<%=timeKey%>" value="<%=opCodeData%>" class="input_noBorder" style="width:20px;" readonly onClick="showOpBtn('<%=timeKey%>');" />
                            <input type="button" name="opSel<%=timeKey%>" value="…" style="height:18px;width=20px;display:none;background-color:#fff0f5;" onclick="showOpSelectWin('<%=timeKey%>');" />
                        </td>
                        <!-- 원인부서 -->
                        <td class="td_standard">
                        ...
                        <td class="td_standard">
                            ...
                            <input name="basisSel<%=timeKey%>" value='<%=basisData%>' class="input_small" style="width:200px;height:18px;display:none;background-color:#fff0f5;" onChange="basisSelChanged('depart', '<%=timeKey%>');" onkeydown="inputCtrlKeydownHandler();" />
                        </td>
                        <!-- 업무내용 -->
                        <td class="td_standard">
                            <input name="workContent<%=timeKey%>" value='<%=workDescData%>' class="input_noBorder" style="width:295px;" readonly onClick="showWorkContentSel('workContent', '<%=timeKey%>');" />
                            <input name="workContentSel<%=timeKey%>" value='<%=workDescData%>' class="input_small" style="width:295px;height:18px;display:none;background-color:#fff0f5;" onChange="elementSelChanged('workContent', '<%=timeKey%>');" onkeydown="inputCtrlKeydownHandler();" />
                        </td>
                        <!-- EVENT1 -->
                        <td class="td_standard">
...
<script language="javascript">
    ...
    function showOpSelectWin(timeKey) 
    {
        ...
        var sProperties = 'dialogHeight:440px;dialogWidth:440px;scroll=no;center:yes;resizable=no;status=no;';
        var opCode = window.showModalDialog("stxPECDPInput_OpSelect_SP.jsp?projectNo=" + projectNo, "", sProperties);
        ...
        if (opCode != null && opCode != 'undefined') 
        {
            ...
            if (projectNo != '' && projectNo != 'S000' && drawingNo != '' && drawingNo != '*****' && drawingNo != '#####') 
            {
                ...
                {
                    // 해양호선의 경우(호선명이 H/M/F로 시작): 도면개정(OP CODE 5*)만 선택 가능(Added on 2012-02-09)
                    var pjtNoPrefixStr = projectNo.substring(0, 1);
                    if ((pjtNoPrefixStr == 'H' || pjtNoPrefixStr == 'M' || pjtNoPrefixStr == 'F') && pjtNoPrefixStr != 'M1101') // M1101은 해양호선이 아님
                    {
                        ...
                        if (date1 - date2 > 0 && opCode != '25' && opCode.substring(0, 1) != '4' && opCode.substring(0, 1) != '5') {
                            var msg = drawingNo + " 도면은 " + workingStartDate + " 에 이미 출도가 되었습니다.\n\n";
                            msg += "해양호선의 경우 출도날짜 입력 후 설계시수 입력 시에는\n\n";
                            msg += "OP CODE 25 또는 4, 5로 시작하는 코드 입력할 수 있습니다!";
                            alert(msg);
                            return;
                        }
                    }
                    // 해양호선 외: 개정에 해당하지 않는 도면작업(OP CODE 21 ~ 24)은 선택 불가
                    else if (opCode == "21" || opCode == "22" || opCode == "23" || opCode == "24") 
                    {
                        var dateStr1 = DPInputMain.dateSelected.value;
                        var dateStrs1 = dateStr1.split("-");
                        var date1 = new Date(dateStrs1[0], dateStrs1[1] - 1, dateStrs1[2]); // 시수입력일
            ...
            // OP CODE가 '5x'가 아닌 경우 '원인부서' 입력 값을 초기화한다
            ...
            // OP CODE가 '20' & '5x'가 아닌 경우 '근거' 입력 값을 초기화한다
            if (opCode.substring(0, 1) != '5' && opCode != '20') {
    ...
    function showDrwTypeSel(elemPrefix, timeKey)
    {
        ...
        if (pjtObj.value == '' || pjtObj.value == 'S000') return; // 공사번호(호선) 값이 없거나 S000 이면 입력 컨트롤 표시하지 않는다
    ...
    function showDrwNoSel(elemPrefix, timeKey)
    {
        ...
        if (drwTypeObj.value == '') {
            ...
            if (pjtObj.value == '' || pjtObj.value == 'S000') return; 
    ...
    function showDepartSel(elemPrefix, timeKey)
    {
        ...
        if (opObj.value.substring(0, 1) == '5') { // OP CODE가 도면수정(5A~5R) 인 경우에만 입력컨트롤을 표시한다
            var departSelObj = DPInputMain.all('departSel' + timeKey);
    ...
    {
        var opObj = DPInputMain.all('op' + timeKey);
        if (opObj.value == "20" || opObj.value.substring(0, 1) == '5') { // OP CODE가 선주 Extra(20) 또는 도면수정(5A~5R) 인 경우에만 입력컨트롤을 표시한다
            showElementSel(elemPrefix, timeKey);
        }
    }
    ...
    // projectSelChanged() 실행된 후 호선이 'S000'이면 OP CODE 창을 띄우고, 기타 호선이면 도면타입 선택 컨트롤을 Show 시킨다
    function projectSelChangedAfter(timeKey)
    {
        toggleActiveElementDisplay();

        var pjtObj = DPInputMain.all('pjt' + timeKey);
        if (pjtObj.value == 'S000') showOpSelectWin(timeKey);
    ...
    function drawingTypesForWorkQueryProc(pjtSelObj, drwTypeSelObj, selectedValue)
    {
        ...
        xmlHttp.open("GET", "stxPECDP_GeneralAjaxProcess_SP.jsp?requestProc=GetDrawingTypesForWork&departCode=" + 
    ...
    function drawingListForWorkQueryProc(pjtSelObj, drwTypeValue, drwNoSelObj, selectedValue)
    {
        ...
        xmlHttp.open("GET", "stxPECDP_GeneralAjaxProcess_SP.jsp?requestProc=GetDrawingListForWork&departCode=" + 
    ...
    function checkInputs()
    {
        ...
        if (dpInputLockDateStr == "ERROR") {
            ...
            if (parent.DP_HEADER.DPInputHeader.isAdmin.value != "true" && (selectedDate - dpInputLockDate < 0)) {
                //alert("입력 유효일자(기본: 해당일자 + Workday 2일)가 경과되었습니다.\n\n먼저 관리자에게 LOCK 해제를 요청하십시오.");
                
                var alertMsg = "입력 유효일자(기준: 당일 - 2 Working day)가 경과되었습니다.\n\n";
                alertMsg += "시수입력 제한 해지 요청서 작성하시어 결재를 득한 후 \n\n";
                alertMsg += "특수선 PLM 관리자에게 LOCK 해제를 요청 하십시오.";
                alert(alertMsg);
        ...
        for (var i = 0; i < timeKeys.length; i++) {
            ...
            if (errKey == '' && (op == '20' || op.substring(0, 1) == '5')) { 
            ...
            if (errKey == '' && pjt != 'S000' && pjt != 'V0000' && drwNo != "*****" && drwNo != "#####" 
                && op != "45" && op != "95" && event1Value == "") errKey = "Event1";
    ...
    function saveDPInputsProc(inputDoneYN, showMessage)
    {
        ...
        for (var i = 0; i < timeKeys.length; i++) {
            ...
            if (i == 0 && (opCode == '97' || opCode == '93' || opCode == '94')) { 
            ...
            // 일반적인 케이스 (시운전 포함)
                ...
                else if (opCode == '45' && projectNo != 'V0000') { // 시운전은 뒤에 다른 항목이 있으면 위의 일반 케이스로 처리됨, 없으면 연장근무 3 시간 기준으로 처리(휴일은 특근 8시간)
                    if (workingDayYN == 'Y') { 
                ...
                if (opCode == '90') inputDoneYN = 'Y'; // 퇴근이면 시수입력 완료
            }
    ...
    function saveAsVacationProc(opCode)
    {
        ...
        var periodStr = window.showModalDialog("stxPECDPInput_VacationPeriodSelect_SP.jsp?opCode=" + opCode, "", sProperties);
        ...
        if (dpInputLockDateStr == "ERROR") {
            ...
            if (parent.DP_HEADER.DPInputHeader.isAdmin.value != "true" && (fromDate - dpInputLockDate < 0)) {
                //alert("입력 유효일자(기본: 해당일자 + Workday 2일)가 경과되었습니다.\n\n먼저 관리자에게 LOCK 해제를 요청하십시오.");
        ...
        var jobDescStr = "?????";
        if (opCode == "93") jobDescStr = "예비군훈련";
        else if (opCode =="94") jobDescStr = "특별휴가";
        else if (opCode == "97") jobDescStr = "년차";
    ...
    function saveAsOneDayOverJob(opCode)
    {
        ...
        var returnVal = window.showModalDialog("stxPECDPInput_OneDayOverJobPeriodSelect_SP.jsp?opCode=" + opCode, "", sProperties);
        ...
        var jobDescStr = "?????";
        if (opCode == "72") jobDescStr = "기술회의 및 교육(사내외)";
        else if (opCode == "81") jobDescStr = "일반출장(기술소위원회, 세미나)";
    ...
    function saveAsOneDayOverJobWithProject(opCode)
    {
        ...
        var returnVal = window.showModalDialog("stxPECDPInput_OneDayOverJobPeriodSelectPjt_SP.jsp?" + paramStr, "", sProperties);
        ...
        var jobDescStr = "?????";
        if (opCode == "36") jobDescStr = "사외 협의 검토(공사관련 출장)";
    ...
    function finishWork()
    {
        ...
        for (var i = timeKeys.length - 1; i >= 0; i--) {
            ...
            if (editWinStyle.display != 'none') {
                ...
                if (!inputExist && DPInputMain.all('op' + timeKeys[i]).value != "" && DPInputMain.all('op' + timeKeys[i]).value != "90") inputExist = true;
                if (inputExist) {
                    ...
                    DPInputMain.all('op' + timeKeys[i]).value = "90"; // 90 : 퇴근
    ...
    function finishWorkEarly()
    {
        ...
        for (var i = timeKeys.length - 1; i >= 0; i--) {
            ...
            if (editWinStyle.display != 'none') {
                ...
                var inputExist = false;
                if (DPInputMain.all('pjt' + timeKeys[i]).value != "" && DPInputMain.all('pjt' + timeKeys[i]).value != "S000") inputExist = true;
                if (!inputExist && DPInputMain.all('op' + timeKeys[i]).value != "" && DPInputMain.all('op' + timeKeys[i]).value != "96") inputExist = true;
                if (inputExist) {
                    ...
                    DPInputMain.all('drwType' + timeKeys[i]).value = ""; DPInputMain.all('drwTypeSel' + timeKeys[i]).selectedIndex = 0;
                    DPInputMain.all('drwNo' + timeKeys[i]).value = "*****"; DPInputMain.all('drwNoSel' + timeKeys[i]).selectedIndex = 0;
                    DPInputMain.all('op' + timeKeys[i]).value = "96"; // 96 : 조퇴
                    DPInputMain.all('depart' + timeKeys[i]).value = ""; if (DPInputMain.all('departSel' + timeKeys[i]).options.length > 0) DPInputMain.all('departSel' + timeKeys[i]).selectedIndex = 0;
                    DPInputMain.all('basis' + timeKeys[i]).value = ""; DPInputMain.all('basisSel' + timeKeys[i]).value = "";
                    if (DPInputMain.all('workContent' + timeKeys[i]).value.trim() == "") {
    ...
    function saveAsSeaTrial()
    {
        ...
        var paramStr = "designerID=" + DPInputMain.designerID.value;
        var periodStr = window.showModalDialog("stxPECDPInput_SeaTrialPeriodSelect_SP.jsp?" + paramStr, "", sProperties);
        ...
        if (dpInputLockDateStr == "ERROR") {
            ...
            if (parent.DP_HEADER.DPInputHeader.isAdmin.value != "true" && (seaTrialFromDate - dpInputLockDate < 0)) {
                //alert("입력 유효일자(기본: 해당일자 + Workday 2일)가 경과되었습니다.\n\n먼저 관리자에게 LOCK 해제를 요청하십시오.");
        ...
        if (DPInputMain.dateSelected.value == fromDate) 
        {
            ...
            DPInputMain.all('op' + fromTime).value = "45"; // 45 : 시운전
            ...
            if (fromDate == toDate) {
                ...
                if ((workingDayYN == "Y" && index > 26) || (workingDayYN == "N" && index > 18) || (workingDayYN == "4H" && index > 16)) {
                    ...
                    DPInputMain.all('op' + toTime).value = "90"; // 90 : 퇴근
        ...
        else 
        {
            ...
            xmlHttp.open("GET", "stxPECDP_GeneralAjaxProcess_SP.jsp?requestProc=SaveSeaTrialDPInputs&designerID=" + DPInputMain.designerID.value + 
        ...
        if (fromDate != toDate) {
            ...
            params += "&timeKey0800=0800&pjt0800=" + projectNo + "&drwType0800=&drwNo0800=*****&op0800=45&depart0800=&basis0800=&workContent0800=시운전";
            ...
            if (!finishEarlyYN) 
            {
                ...
                else 
                {
                    ...
                    params += "&op" + toTime + "=90&depart" + toTime + "=&basis" + toTime + "=&workContent" + toTime + "=퇴근";
            ...
            // 시운전 후 조기퇴근으로 적용
                ...
                if (timeIndex != 0) {
                    ...
                    params += "&timeKey" + toTime + "=" + toTime + "&pjt" + toTime + "=S000&drwType" + toTime + "=&drwNo" + toTime + "=*****";
                    params += "&op" + toTime + "=95&depart" + toTime + "=&basis" + toTime + "=&workContent" + toTime + "=시운전 후 조기퇴근";
                    params += "&event1" + toTime + "=&event2" + toTime + "=&event3" + toTime + "=";
                    ...
                    params = "";
                    params = "designerID=" + DPInputMain.designerID.value + "&dateStr=" + toDate + "&loginID=" + DPInputMain.loginID.value;
                    params += "&timeKey0800=0800&pjt0800=S000&drwType0800=&drwNo0800=*****&op0800=95&depart0800=&basis0800=&workContent0800=시운전 후 조기퇴근";
                    params += "&event10800=&event20800=&event30800=";
                    params += "&normalTime0800=" + normalTime + "&overtime0800=0&specialTime0800=0";
                    params += "&inputDoneYN=Y";
                }
    ...
    function finishWorkEarlyAfterSeaTrial(comments)
    {
        ...
        for (var i = timeKeys.length - 1; i >= 0; i--) {
            ...
            if (editWinStyle.display != 'none') {
                if (DPInputMain.workingDayYN.value == '평일' && i >= 20) {
                    alert("18:00 이후에는 '"+comments+"' 코드를 입력할 수 없습니다.\n\n입력 시각을 확인하여 주십시오.");
                    return;
                }
                if (DPInputMain.workingDayYN.value == '4H' && i >= 8) {
                    alert("12:00 이후에는 '"+comments+"' 코드를 입력할 수 없습니다.\n\n입력 시각을 확인하여 주십시오.");
                    return;
                }
                var inputExist = false;
                if (DPInputMain.all('pjt' + timeKeys[i]).value != "" && DPInputMain.all('pjt' + timeKeys[i]).value != "S000") inputExist = true;
                if (!inputExist && DPInputMain.all('op' + timeKeys[i]).value != "" && DPInputMain.all('op' + timeKeys[i]).value != "95") inputExist = true;
                if (inputExist) {
                    alert("적용할 시각을 먼저 선택한 후 '"+comments+"'을 적용하십시오.");
                    return;
                }
                else {
                    DPInputMain.all('pjt' + timeKeys[i]).value = "S000"; DPInputMain.all('pjtSel' + timeKeys[i]).selectedIndex = 1;
                    DPInputMain.all('drwType' + timeKeys[i]).value = ""; DPInputMain.all('drwTypeSel' + timeKeys[i]).selectedIndex = 0;
                    DPInputMain.all('drwNo' + timeKeys[i]).value = "*****"; DPInputMain.all('drwNoSel' + timeKeys[i]).selectedIndex = 0;
                    DPInputMain.all('op' + timeKeys[i]).value = "95"; // 95 : 시운전 후 조기퇴근
                    DPInputMain.all('depart' + timeKeys[i]).value = ""; if (DPInputMain.all('departSel' + timeKeys[i]).options.length > 0) DPInputMain.all('departSel' + timeKeys[i]).selectedIndex = 0;
                    DPInputMain.all('basis' + timeKeys[i]).value = ""; DPInputMain.all('basisSel' + timeKeys[i]).value = "";
                    if (DPInputMain.all('workContent' + timeKeys[i]).value.trim() == "") {
                        DPInputMain.all('workContent' + timeKeys[i]).value = comments; DPInputMain.all('workContentSel' + timeKeys[i]).value = comments;
                    }
                    DPInputMain.all('event1' + timeKeys[i]).value = ""; DPInputMain.all('event1Sel' + timeKeys[i]).selectedIndex = 0;
                    DPInputMain.all('event2' + timeKeys[i]).value = ""; DPInputMain.all('event2Sel' + timeKeys[i]).selectedIndex = 0;
                    DPInputMain.all('event3' + timeKeys[i]).value = ""; DPInputMain.all('event3Sel' + timeKeys[i]).selectedIndex = 0;
    ...
    function copyYesterdayMH()
    {
        ...
        xmlHttp.open("GET", "stxPECDP_GeneralAjaxProcess_SP.jsp?requestProc=GetWorkingDayDesignMHInputs" + 
    ...
    function deleteDPInputs()
    {
        ...
        if (confirm(msg)) {
            ...
            xmlHttp.open("GET", "stxPECDP_GeneralAjaxProcess_SP.jsp?requestProc=DeleteDPInputs&designerID=" + DPInputMain.designerID.value + 
    ...
    function printPage()
    {
        var urlStr = "http://172.16.2.13:7777/j2ee/STXDP/WebReport.jsp?src=http://172.16.2.13:7777/j2ee/STXDP/mrd/stxPECDPInput.mrd&param=" +
                     "<%=designerID%>:::" + "<%=dateSelected%>" ;
        window.open(urlStr, "", "");
    }
