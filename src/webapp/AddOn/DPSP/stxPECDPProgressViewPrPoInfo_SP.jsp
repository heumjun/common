...
<%!
	...
	// Vendor 도면 일정관리 - 해당 도면의 본 PR 추출
	private String getOriginPrNo(String projectNo,String drawingNo) throws Exception
	{
		if (StringUtil.isNullString(projectNo)) throw new Exception("Project No. is null");
        if (StringUtil.isNullString(drawingNo)) throw new Exception("drawingNo is null");
        ...
        try {
            ...
            strQuery.append("SELECT PR_NO                                        \n"); 
            strQuery.append("  FROM STX_PO_EQUIPMENT_DP                          \n"); 
            strQuery.append(" WHERE PROJECT_NUMBER   = '"+projectNo+"'           \n");
            strQuery.append("   AND DRAWING_NO       = '"+drawingNo+"'           \n");
            strQuery.append("   AND ORIGINAL_PR_FLAG = 'Y'                       \n");
...
<%
    ...
    String prNo = StringUtil.setEmptyExt(emxGetParameter(request, "prNo"));  
    ...
    ArrayList prPoInfoList = new ArrayList();
    ArrayList prStateList = new ArrayList();
    ...
    try {
	  	...
	  	if("".equals(prNo))
	    {	  		
	  		prNo = getOriginPrNo(projectNo, drawingNo);	    	  // 파라미터로 넘겨진 PR NO.가 없을 경우(공정조회 화면에서 팝업 시) 본 PR NO 추출	
	    }
	  	
	  	if(!"".equals(prNo))
	  	{
	
	        drawingInfoHeader = getDrawingInfo(projectNo, prNo, drawingNo);    // 도면 NO TITLE 추출. 헤더 표기 명칭
	
	        prStateList = getPrState(prNo);                                    // PR 상태 추출
	
	        prPoInfoList = getPrPoInfo(projectNo, prNo);                       // PR PR 정보 추출
	        
	        prAPPROVER = searchPRApprover(personId);                           // PR 결재자 정보 추출 (인사상 지정된 상위 결재자)	        
...
<% 
// PR No가 있을 경우만 화면보여주고 없으면 화면 Close.
if(!"".equals(prNo))
{
%>
    ...
    %>
        <tr height="45">
            ...
            <td style="text-align:right;">             
                <input type="button" value="닫기" class="button_simple" onclick="javascript:parent.window.close();">&nbsp;
            </td>
        </tr>
        <tr height="35">
