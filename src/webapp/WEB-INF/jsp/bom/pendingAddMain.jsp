...
<%
	String deptCode = (request.getParameter("p_dept_code") == null ||  request.getParameter("p_dept_code").equals("")) ? "" : request.getParameter("p_dept_code");
%>
<!DOCTYPE html>
<html>
<head>
...
<form id="application_form" name="application_form"  >
	...
	<input type="hidden" name="p_li_seq_id" id="p_li_seq_id" value="" />
	<input type="hidden" name="p_dept_code" value="<%=deptCode %>" />
	<input type="hidden" name="p_ischeck" value="Y" />
	...
	<div class="mainDiv" id="mainDiv">
		...
		<col width="*"/>
			<tr>
				<th>PROJECT</th>
				<td>
					<input type="text" class="required" name="p_project_no" alt="Project"  style="width:70px;" value="${p_project_no}"  readonly="readonly" />
				</td>
			...
			<tr>
				<td>
					<div>
						...
						<input type="button" class="btn_red2" value="+" id="btnAdd" style="width:28px;"/>
						<input type="button" class="btn_red2" value="-" id="btnDel" style="width:28px;"/>
					</div>	
				</td>
				<td class="bdl_no"  style="border-left:none;">
					<div class="button endbox" >
						<input type="button" class="btn_blue2" value="DELETE" id="btnDel2"/>
		...
		<div id="divList" class="content" style="position:relative; float: left; width: 43%;">
		...
		<div id="divList1" class="content" style="position:relative; float: right; width: 53%;">
...
<script type="text/javascript" charset='utf-8'>
	...
	$(function() {
		...
		//getAjaxTextPost(null, "getAutoCompleteJobCatalogInfo.do", form, getJobCatalogCallback);
	  	getAjaxTextPost(null, "getDwgNoList.do", form, getdwgnoCallback);
	  	
	});
	
	var dwgOnFocus = function(){
	...
	$(document).ready(function(){
		...
		//시리즈 호선 받기		
		getAjaxHtml($("#SeriesCheckBox"),"sscCommonSeriesCheckBox.do?p_item_type_cd="+$("input[name=p_item_type_cd]").val()+"&p_project_no="+$("input[name=p_project_no]").val()+"&p_ischeck="+$("input[name=p_ischeck]").val()+"&p_chk_series="+$("input[name=p_chk_series]").val(), null, getDeliverySeriesCallback);
		...
		//STR 받기
		getAjaxHtml($("#p_str"),"commonSelectBoxDataList.do?sb_type=all&p_query=getJobStr", null, null);
		
		// /js/getGridColModel.js에서 그리드의 colomn을 받아온다.
		jqGridObj.jqGrid({ 
            datatype: 'json',
            url:'pendingAddList.do',
            mtype : 'POST',
			...
			gridComplete: function () {
				...
				$("td:eq(14)", $("#jqGridMainList tr")).mouseover(function(e) {
					$(this).css("text-decoration", "underline");
					$(this).css("cursor", "pointer");
				});
				$("td:eq(14)", $("#jqGridMainList tr")).mouseout(function (e) {
				...
				for ( var i = 0; i < rows.length; i++ ) {
					...
					if( oper == "I" ) {
						jqGridObj.jqGrid( 'setCell', rows[i], 'project_no', '', { cursor : 'pointer', background : '#FFFFA2' } );
						jqGridObj.jqGrid( 'setCell', rows[i], 'block_no', '', { cursor : 'pointer', background : '#FFFFA2' } );
						jqGridObj.jqGrid( 'setCell', rows[i], 'str_flag', '', { cursor : 'pointer', background : '#FFFFA2' } );
						jqGridObj.jqGrid( 'setCell', rows[i], 'job_catalog', '', { cursor : 'pointer', background : '#FFFFA2' } );
						jqGridObj.jqGrid( 'setCell', rows[i], 'dwg_no', '', { cursor : 'pointer', background : '#FFFFA2' } );
					}
				}
		...
		$("#btnNext").click(function(){
			...
			getGridChangedData(jqGridObj,function(data) {
				...
				var arr = (''+jqGridObj.jqGrid("getDataIDs")).split(',');
				var rtn = false;
				$(arr).each(function(i) {
					if(jqGridObj.getCell(arr[i], "block_no") == '' || jqGridObj.getCell(arr[i], "str_flag") == '' 
							|| jqGridObj.getCell(arr[i], "job_catalog") == '' || jqGridObj.getCell(arr[i], "dwg_no") == '') {
						rtn = true;
					}
				});
				
				if(rtn) {
					alert("필수 미입력 값이 존재합니다.");
					return false;
				}
				...
				getJsonAjaxFrom("pendingWorkInsert.do?p_check_series="+ar_series, parameters, null, callback_next);
		...
		$("#btnApply").click(function(){
			...
			if(confirm('적용하시겠습니까?')){
				...
				$.post("pendingApplyAction.do", formData, function(data){
		...
		$("#btnImport").click(function(){
			...
			var sURL = "pendingAddWorkExcelImport.do";
		...
		$("#btnAdd").click(function(){
			...
			item.job_catalog = rowData['job_catalog'];
			item.usc_job_type = rowData['usc_job_type'];
			item.ship_type = rowData['ship_type'];
			item.mode = 'D';
		...
		$("#btnDel").click(function(){
			...
			var selarrrow = jqGridObj.jqGrid('getGridParam', 'selarrrow');
			var selarrrow_len = selarrrow.length;
			
			for(var i=0; i<selarrrow_len; i++) {
				jqGridObj.jqGrid('delRowData', selarrrow[0]);
			}
		...
		$("#btnDel2").click(function(){
			...
			var p_li_seq_id = new Array();
			var selarrrow = $("#pendingDetail").jqGrid('getGridParam', 'selarrrow');
			
			for(var i=0; i<selarrrow.length; i++) {
				var item = $("#pendingDetail").jqGrid( 'getRowData', selarrrow[i]);
				p_li_seq_id.push(item.li_seq_id);
			}
			
			$("#p_li_seq_id").val(p_li_seq_id);
			
			var formData = fn_getFormData('#application_form');
			getAjaxTextPost(null, "pendingWorkDelete.do", formData, callback_next);
		});
	...
	//DWG NO 링크 연결.
	var getDwgnoList = function(){
		
		var sURL = "popupPendingAddGetDwgno.do?p_project_no="+$("input[name=p_project_no]").val()
				+"&p_dept_code="+$("input[name=p_dept_code]").val();
		var popOptions = "dialogWidth: 660px; dialogHeight: 500px; center: yes; resizable: yes; status: no; scroll: yes;"; 
		window.showModalDialog(sURL, window, popOptions);
	}
	...
	function setChangeCata(rowId){

		jqGridObj.saveCell(kRow, idCol );
		var item = jqGridObj.jqGrid( 'getRowData', rowId );
		jqGridObj.jqGrid( 'setCell', rowId, 'usc_job_type', item.job_catalog );
	}
