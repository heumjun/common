				...
				<tr>
					<td >
						<div class="button endbox">
								<input type="button" value="결과서ADD" 	id="btnAdd" 	class="btn_blue2"/>
								<input type="button" value="결과서DEL" 	id="btnDelete" 	class="btn_blue2"/>
								<input type="button" value="UPLOAD"		id="btnUpload" 	class="btn_blue2"/>
...
$(document).ready(function(){
	...
	gridObj.jqGrid({ 
        datatype: 'json', 
        mtype: 'POST', 
        url:'popUpPurchasingNewSpecList.do',
	...
	$("#btnAdd").click(function(){
		var newData = [{
			project_no				:	$("#p_master").val()
			,dwg_no					:	$("#p_dwg_no").val()
			,vendor_site_nm 		: 	''
			,po_files_exists_flag 	:	'N'
			,creation_date			:	''
			,act_comment			:	''
			,complete_meaning		:	''
			,ds_files_exists_flag   :	'N'
			,ds_creation_date		:	''
			,ds_act_comment			:	''
			,spec_review_id			:	''
			,ds_spec_review_id		:	''
	...
	$("#btnUpload").click(function(){
		var row_id = gridObj.jqGrid('getGridParam', 'selarrrow');
		if(row_id == ""){
			alert("행을 선택해 주십시오.");
			return false;
		}
		...
		var master = $("#p_master").val();
      	...
      	var url = "popUpPurchasingNewSpecUpload.do?p_master=" + master + "&p_dwg_no=" + dwg_no + "&p_row_id=" + row_id;
	...
	$("#btnDelete").click(function(){
		...
		var item = gridObj.jqGrid('getRowData',row_id);
		if(!($.trim(item.ds_spec_review_id) == "" && $.trim(item.spec_review_id) == "")){
			alert("해당 건은 삭제하실 수 없습니다.");
			return;
		}
		gridObj.jqGrid('delRowData',row_id[0]);
	});
	...
	$("#btnApply").click(function(){
		...
		var row_id = gridObj.jqGrid('getGridParam', 'selarrrow');
		if(row_id == ""){
			alert("선택된 행이 없습니다. 행을 선택하십시오.");
			return false;
		}
		
		for(var i= 0; i < row_id.length; i++){
			var rowData = gridObj.jqGrid('getRowData',row_id[i]);
			if($.trim(rowData.complete_meaning) == ""){
				alert("Flag 상태값을 지정하여주십시오.");
				return;
			} 
			if(rowData.ds_files_exists_flag == "N"){
				alert("선택된 건에 대한 파일을 업로드 해주십시오.");
				return;
			}
			if($.trim(rowData.ds_spec_review_id) == "" && $.trim(rowData.spec_review_id) == "" && $.trim(rowData.complete_meaning) != "999"){
				alert("결과서ADD한 항목은 FLAG 상태값이 결과만 허용됩니다.");
				return;
			}
		}
		...
		var chmResultRows = [];
		getChkedChmResultData(gridObj,function(data) {
			
			chmResultRows = data;
			
			var url = "popUpPurchasingNewSpecApply.do";
			...
			$.post(url, parameters, function(data) {
				...
				if(data.result == "success") {
					gridObj.setGridParam({ page: 1, datatype: "json" }).trigger('reloadGrid');
				}
...
function fileUpload(file_ids, rowId) {
	gridObj.jqGrid('setCell',rowId, 'ds_files_exists_flag','Y');
	gridObj.jqGrid('setCell',rowId, 'ds_file_ids',file_ids);
}
