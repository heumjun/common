...
<form name="listForm" id="listForm"  method="get">
		...
		<!-- <input type="hidden"  name="selected_tab"  	value=""/> -->
		<!-- <input type="hidden"  name="area_code" 	 	value=""/> -->
		<input type="hidden"  name="block_code"  	value=""/>
		<input type="hidden"  name="isLastRev"   	value=""/>
		<input type="hidden"  name="revStates"   	value=""/>
...
$(document).ready(function(){
	...
	/* $("#teamIframe").attr("src","paintOutfittingTab.do?teamCode=1&menu_id=${menu_id}&up_link=${up_link}");
	...
	$("#itemGroupList").jqGrid({ 
             ...
             colNames:['<input type="checkbox" id = "chkHeader" onclick="checkBoxHeader(event)" />','PAINT CODE','PAINT DESC','QUANTITY','THEORY QUANTITY',''],
                colModel:[
                   	{name:'chk', 			index:'chk', 			 width:20,		hidden:true, sortable:false, align:'center',formatter: formatOpt1},
            ...
            // width		: 1350,                     //사용자 화면 크기에 맞게 자동 조절
	         ...
	         afterSaveCell  : function(rowid,name,val,iRow,iCol) {
             	...
             	if ("quantity" == name) {
             		//var  nTheoryQty = Math.round(Number(val) / 3 * 100) / 100;
             		
             		//$("#itemGroupList").jqGrid("setCell", rowid, 'theory_quantity', nTheoryQty);	
             	}
	...
	//fn_gridresize($(window),$("#fieldsetItemGroup"));
...
function changeTab(teamCode) {
	$("input[name=team_count]").val(teamCode);
	$("#teamIframe").attr("src","paintOutfittingTab.do?teamCode="+teamCode+"&menu_id=${menu_id}");
	//var ifraId = fn_searchIfraId();
	//fn_ifraSetButtionEnable(ifraId);
	fn_search();
}
...
function addRow(addGbn, sPaintCode, sPaintDesc, nQty, nTheoryQty) {
	...
	if (rowid == null) {
		...
		item.theory_quantity = Math.round(Number(nQty) / 3 * 100) / 100;
...
function fn_checkOutfittingQuantityValidate(arr1) {
	var result   = true;
	var message  = "";
...
/* function fn_tabGridClear() {
	...
	var ifra = document.getElementById('prePaintingTeam').contentWindow;
	ifra.resetGrid();
	
	ifra = document.getElementById('paintingTeamAft').contentWindow;
	ifra.resetGrid();
	
	ifra = document.getElementById('paintingTeamMid').contentWindow;
	ifra.resetGrid();
	
	ifra = document.getElementById('paintingTeamFwd').contentWindow;
	ifra.resetGrid();
	
	ifra = document.getElementById('specailPaintingTeam').contentWindow;
	ifra.resetGrid();
	
	ifra = document.getElementById('machinariTeam').contentWindow;
	ifra.resetGrid();
	
	ifra = document.getElementById('hullOutfittingTeam').contentWindow;
	ifra.resetGrid();
		
	ifra = document.getElementById('deckHouseOutfitting').contentWindow;
	ifra.resetGrid();
} */

/* // iframe 그리드 컬럼 show, hide하는 함수 
function fn_ifraGridChkColumn(ifraId) {
	var ifra = document.getElementById(ifraId).contentWindow;
	ifra.fn_chkColumnHide();
} */

// 선택되어있는  iframe객체 리턴하는 함수
/* function fn_tabSearch(){
	
	var tabId = fn_searchIfraId();
	return document.getElementById(tabId).contentWindow;
...
/* function fn_searchIfraId(){
	...
	switch(selected_tab_name) {
		case "#tabs-1":
			ifraId = "prePaintingTeam";
			break;
		case "#tabs-2":
			ifraId = "paintingTeamAft";
			break;
		case "#tabs-3":
			ifraId = "paintingTeamMid";
			break;
		case "#tabs-4":
			ifraId = "paintingTeamFwd";
			break;
		case "#tabs-5":
			ifraId = "specailPaintingTeam";
			break;
		case "#tabs-6":
			ifraId = "machinariTeam";
			break;
		case "#tabs-7":
			ifraId = "hullOutfittingTeam";
			break;
		case "#tabs-8":
			ifraId = "deckHouseOutfitting";
			break;							
	}
...
function fn_ifraSetButtionEnable(ifraId) {
	//if (ifraId == "machinariTeam") {
		alert(ifraId);
		var ifra = ifraId.contentWindow;
...
function fn_setSearchGbn(sGbn,sPaintGbn) {
	...
	switch(sGbn) {
		...
		case "shopPrimer":	
			sPaintGbn = "SPR";
			break;		
		case "DeckHouseShopPrimer":
			sPaintGbn = "SPR";
			break;
		case "exteriorMarking":	
			sPaintGbn = "EMK";
			break;
		case "ballastGalvPipe":	
			sPaintGbn = "BGP";
			break;
		case "rudderHole":	
			sPaintGbn = "RUH";
			break;
		case "loAndHydroPipeMarking":	
			sPaintGbn = "LHP";
			break;
		case "aftUpperDeckFinish":
		case "midUpperDeckFinish":
		case "fwdUpperDeckFinish":		
			sPaintGbn = "UDF";
			break;
		case "exteriorFullCoat":	
			sPaintGbn = "EFC";
			break;
		case "bowEmblemAndShipNameAndSideMark":	
			sPaintGbn = "BSS";
			break;
		case "condensatePipe":	
			sPaintGbn = "CNP";
		...
		case "spillTank":	
			sPaintGbn = "SPT";
			break;	
		case "outerPlateMarking":	
			sPaintGbn = "OPM";
			break;
		case "uppDkOutfittingAndMarking":	
			sPaintGbn = "UDO";
			break;
		case "foremastAndCreane":	
			sPaintGbn = "FCR";
			break;
		case "megongWelding":	
			sPaintGbn = "MWD";
			break;
		case "specialPainting":	
			sPaintGbn = "SPT";
			break;
		case "cargoTankFloor":	
			sPaintGbn = "CTF";
		...
		case "exhaustGasPipe":	
			sPaintGbn = "EGP";
			break;
		case "mechanicalEquipmentMaintenance":	
			sPaintGbn = "MEM";
			break;	
		case "valveMarking":	
			sPaintGbn = "VMK";
		...
		case "alAnodePutty":	
			sPaintGbn = "APU";
			break;
		case "sideLightUscgBoxExhaustGasPipe":	
			sPaintGbn = "SUE";
			break;
		case "funnelMarking":	
			sPaintGbn = "FUM";
			break;
		case "ExternalEquipmentAndFireFoamLine":	
			sPaintGbn = "EEF";
			break;
		case "keepChainAndFireHydrantValve":	
			sPaintGbn = "KFV";
			break;
		case "marking":	
			sPaintGbn = "MRK";
...
function fn_search() {
	...
	var sUrl = "infoOutfittingItemGroupList.do";
...
function fn_searchOutfittingArea() {
	...
	var url		    = "selectOutfittingArea.do";
 	var parameters  = fn_getFormData('#listForm');
...
// 		var obj  = jQuery.parseJSON(data);
		var ifra = fn_tabSearch();
	  	...
	  	if (data.length > 0) {
	  		if( data[0] == null ) {
	  			ifra.fn_setOutfittingArea("");
	  		} else {
		  		ifra.fn_setOutfittingArea(data[0].area);
	  		}
	  		//return  obj[0].area;
	  	} else {
	  		ifra.fn_setOutfittingArea("");
...
/* function fn_save() {
	...
	if (!fn_checkOutfittingQuantityValidate(changePaintItemResultRows)) { 
		lodingBox.remove();
		return;	
	}
	
	var url			= "saveOutfitting.do";
	var dataList    = {outfittingList:JSON.stringify(changePaintItemResultRows)};
...
function fn_excelDownload() {
	...
	f.action = "outfittingExcelExport.do";
	f.method = "post";
	f.submit();
...
function minusPaintItem(addGbn, sPaintCode, sPaintDesc, nQty, nTheoryQty) {
	...
	var url			= "saveMinusOutfitting.do";
	var dataList    = {chmResultList:JSON.stringify(addPaintItemResultRows)};
	var formData 	= fn_getFormData('#listForm');
	var parameters  = $.extend({},dataList,formData);
...
function fn_searchGroupDetail(sItemCode, sItemDesc) {
	...
	var sUrl = "infoOutfittingItemGroupDetail.do";
