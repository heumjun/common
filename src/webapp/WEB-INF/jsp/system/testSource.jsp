...
<html>
	<head>
		...
		<title>TEST SOURCE (Project Management)</title>
	...
	<body>
		<div id="mainDiv" class="mainDiv">
			<div class="subtitle">
				TEST SOURCE (Project Management)
			...
			<form name="listForm" id="listForm">
				...
				<div class="content">
					<div class="content">
						<table id="testSourceList"></table>
						<div id="p_testSourceList"></div>
					</div>
				</div>
			</form>
		</div>
...
// 				shrinkToFit : false,
				...
				afterSaveCell  : function(rowid,name,val,iRow,iCol) {
	            	if (name == "project_no") setUpperCase("#testSourceList",rowid,name);	
		         },
					...
					if(!is_paint_user){
						if ( colName['index'] == "model_no" ) {
							...
							if( item.category == "" ) {
								...
								var rs = window.showModalDialog( "popUpTestSource.do",
										window,
			...
			//grid resize
			fn_gridresize( $(window), $( "#testSourceList" ) );

			$( "#testSourceList" ).jqGrid( 'navGrid', "#p_testSourceList", {
				search : false,
				edit : false,
				add : false,
				del : false
			...
			$( "#testSourceList" ).navButtonAdd( '#p_testSourceList', {
				caption : "",
...
// 			}, "json" );
			...
			$( "#btnSave" ).click( function() {
				$( '#testSourceList' ).saveCell( kRow, idCol );

				var changedData = $( "#testSourceList" ).jqGrid( 'getRowData' );

				// 변경된 체크 박스가 있는지 체크한다.
				for( var i = 1; i < changedData.length + 1; i++ ) {
					var item = $( '#testSourceList' ).jqGrid( 'getRowData', i );
					if( item.oper != 'I' && item.oper != 'U' ) {
						...
						if (item.oper == 'U') {
							...
							$( '#testSourceList' ).jqGrid( "setRowData", i, item );
						}
					}
				}
		...
		function getChangedChmResultData( callback ) {
			var changedData = $.grep( $( "#testSourceList" ).jqGrid( 'getRowData' ), function( obj ) {
		...
		//조회
		function fn_search() {
			var sUrl = "testSourceList.do";

			$( "#testSourceList" ).jqGrid( "clearGridData" );
			$( "#testSourceList" ).jqGrid( 'setGridParam', {
				mtype : 'POST',
				url : sUrl,
				datatype : 'json',
				page : 1,
		...
		function fn_checkValidate() {
			...
			var message = "";
			var nChangedCnt = 0;
			var ids = $( "#testSourceList" ).jqGrid( 'getDataIDs' );

			for( var i = 0; i < ids.length; i++ ) {
				var oper = $( "#testSourceList" ).jqGrid( 'getCell', ids[i], 'oper' );
				if( oper == 'I' || oper == 'U' ) {
					nChangedCnt++;
					var val1 = $( "#testSourceList" ).jqGrid( 'getCell', ids[i], 'project_no' );
					if( $.jgrid.isEmpty( val1 ) ) {
						alert( "Project Name을 입력하십시오." );
						result = false;
						message = "Field is required";
// 						setErrorFocus( "#testSourceList", ids[i], 0, 'project_no' );
						break;
					}
					var val2 = $( "#testSourceList" ).jqGrid( 'getCell', ids[i], 'model_no' );
...
// 						setErrorFocus( "#testSourceList", ids[i], 0, 'project_no' );
		...
		function fn_comboboxload() {
			//참조호선 콤보박스
			$.post( "testSourceProjectSelectBox.do?type=PROJECT", "", function( data ) {
				$( '#testSourceList' ).setObject( {
					value : 'value',
					text : 'text',
					name : 'reference_pro_num',
			...
			//대표호선 콤보박스
			$.post( "testSourceProjectSelectBox.do?type=MASTER", "", function( data ) {
				$( '#testSourceList' ).setObject( {
					value : 'value',
					text : 'text',
					name : 'representative_pro_num',
		...
		//Del 버튼
		function deleteRow() {
			$( '#testSourceList' ).saveCell( kRow, kCol );

			var selrow = $( '#testSourceList' ).jqGrid( 'getGridParam', 'selrow' );
			var item = $( '#testSourceList' ).jqGrid( 'getRowData', selrow );

			if (item.oper == 'I') {
				$( '#testSourceList' ).jqGrid( 'delRowData', selrow );
			} else {
				alert( '저장된 데이터는 삭제할 수 없습니다.' );
			}

			$( '#testSourceList' ).resetSelection();
		}
		...
		function addChmResultRow() {
			...
			var item = {};
			var colModel = $( '#testSourceList' ).jqGrid( 'getGridParam', 'colModel' );
			...
			$( '#testSourceList' ).resetSelection();
			$( '#testSourceList' ).jqGrid( 'addRowData', $.jgrid.randId(), item, 'first' );
		}

		//afterSaveCell oper 값 지정
		function chmResultEditEnd( irowId, cellName, value, irow, iCol ) {
			var item = $( '#testSourceList' ).jqGrid( 'getRowData', irowId );
			if( item.oper != 'I' ){
				item.oper = 'U';
				$( '#testSourceList' ).jqGrid('setCell', irowId, cellName, '', { 'background' : '#6DFF6D' } );
			}
			
			$( '#testSourceList' ).jqGrid( "setRowData", irowId, item );
			$( "input.editable,select.editable", this ).attr( "editable", "0" );

			/* //변경 된 row 이면 색 변경
			var ids = $( '#testSourceList' ).jqGrid( 'getDataIDs' );
			if (item.oper == "U") {
				$( '#testSourceList' ).jqGrid( 'setCell', ids[irow - 1], ids[iCol - 1], '', { 'background' : '#6DFF6D' } );
			} */
		}
		...
		function fn_excelDownload() {
			...
			var cn = $( "#testSourceList" ).jqGrid( "getGridParam", "colNames" );
			var cm = $( "#testSourceList" ).jqGrid( "getGridParam", "colModel" );
			...
			f.action = "testSourceExcelExport.do";
			f.method = "post";
			f.submit();
		}
		</script>
