...
<body>
	<div class="mainDiv" id="mainDiv">
		<form id="application_form" name="application_form">
			...
			<div class="subtitle">담당자관리 <span class="guide"><img src="/images/content/yellow.gif" />&nbsp;필수입력사항</span></div>
	...
	<script type="text/javascript">
						...
						.jqGrid(
								{
									datatype : 'json',
									mtype : 'POST',
									url : 'personInChargeList.do',
									postData : fn_getFormData("#application_form"),
									colNames : ['SEQ', '담당자', '직급', '부서명', '담당업무', '전화번호', '휴대전화', 'E-Mail', 'enable_flag_changed', 'crud'],
									colModel : [
											{name : 'contact_seq', index : 'contact_seq', width : 25, hidden : true},
											{name : 'manager_name', index : 'manager_name', width : 100, editable : false, align : "center"},
											{name : 'position_name', index : 'position_name', width : 100, editable : false, align : "center"},
											{name : 'dept_name', index : 'dept_name', width : 100, editable : false, align : "center"},
											{name : 'manager_task', index : 'manager_task', width : 100, editable : true},
											{name : 'manager_tel', index : 'manager_tel', width : 120, editable : false, align : "center"},
											{name : 'manager_mobile', index : 'manager_mobile', width : 120, editable : false, align : "center"},
											{name : 'email_address', index : 'email_address', width : 120, editable : false, align : "center"},
									...
									loadComplete : function(data) {
										...
										for ( var i = 0; i < rows.length; i++ ) {
											...
											if( oper == "" ) {
												$( "#dataList" ).jqGrid( 'setCell', rows[i], 'manager_name', '', { color : 'black', background : '#DADADA' } );
												...
												//$( "#dataList" ).jqGrid( 'setCell', rows[i], 'manager_task', '', { color : 'black', background : '#DADADA' } );
												$( "#dataList" ).jqGrid( 'setCell', rows[i], 'manager_tel', '', { color : 'black', background : '#DADADA' } );
												$( "#dataList" ).jqGrid( 'setCell', rows[i], 'manager_mobile', '', { color : 'black', background : '#DADADA' } );
												$( "#dataList" ).jqGrid( 'setCell', rows[i], 'email_address', '', { color : 'black', background : '#DADADA' } );
											}
										}
									},
									gridComplete : function(data) {
										...
										for ( var i = 0; i < rows.length; i++ ) {
											...
											var oper = $( "#dataList" ).getCell( rows[i], "oper" );
											if( oper == "I" ) {
												$( "#dataList" ).jqGrid( 'setCell', rows[i], 'manager_name', '', { cursor : 'pointer', background : 'pink' } );
											}
										}
									},
									editurl : "savePersonInCharge.do",
									onCellSelect : function( rowid, iCol, cellcontent, e ) {
										...
										if( oper == "I" ) {
											...
											if ( colName['index'] == "manager_name" ) {
												...
												if( rs != null ) {
													item.contact_seq = rs[0];
													item.manager_name = rs[1];
													item.dept_name = rs[7];
													item.manager_tel = rs[2];
													item.manager_mobile = rs[3];
													item.email_address = rs[4];
													item.manager_task = rs[5];
		...
		function fn_search() {
			...
			var sUrl = "personInChargeList.do";
		...
		function fn_checkValidate() {
			...
			for (var i = 0; i < ids.length; i++) {
				...
				if (oper == 'I' || oper == 'U' || oper == 'D') {
					...
					var val1 = $("#dataList").jqGrid('getCell', ids[i], 'manager_name');

					if ($.jgrid.isEmpty(val1)) {
						result = false;
						message = "담당자 Field is required";

						setErrorFocus("#dataList", ids[i], 0, 'manager_name');
						break;
					}
				}
			}
