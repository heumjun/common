		...
		<div id="searchDiv">
			...
			<table class="searchArea2">
		        <tr>
		            ...
		            <td><font ><b>상근: <span id="normalTimeSum"></span></b></font></td>
		            <td><font ><b>연근: <span id="overTimeSum"></span></b></font></td>
		            <td><font ><b>특근: <span id="specialTimeSum"></span></b></font></td>
		            <td><font ><b>입력율: <span id="inputRateSum"></span>%</b></font></td>
		        </tr>
		    </table>
		</div>
...
$( function() {
	...
	$("#btn_search").click(function(){
		...
		$( '#dataList' ).jqGrid( 'setGridParam', {
			mtype : 'POST',
			url : '<c:url value="popUpApprovalInputRateViewMainGrid.do"/>',
			datatype : 'json',
			page : 1,
...
function viewReport()
{
    ...
    paramStr += application_form.selectRate[1].checked  ? "Y" : "";
...
<script type="text/javascript">
	...
	$(document).ready( function() {
		...
		$( "#dataList" ).jqGrid( {
			...
			url : '',
			postData : fn_getFormData( '#application_form' ),
			colNames : ['부서','사번','직위','성명','상근','연근','특근','입력율'],
			colModel : [
			            	{name : 'dept_name', index : 'dept_name', width: 40, align : "center"},
							{name : 'employee_num', index : 'employee_no', width: 10, align : "center"},
							{name : 'position', index : 'position', width: 10, align : "center"},
							{name : 'emp_name', index : 'emp_name', width: 20, align : "center"},
							{name : 'normal_time', index : 'normal_time', width: 10, align : "center"},
							{name : 'overtime', index : 'overtime', width: 10, align : "center"},
							{name : 'special_time', index : 'special_time', width: 10, align : "center"},
							{name : 'input_ratio', index : 'input_ratio', width: 10, align : "center"}
			           ],
	        gridview : true,
			...
			gridComplete : function() {
				...
				for(var i=0; i<rows.length; i++){
					...
					$('#dataList').jqGrid('setCell',rows[i],'input_ratio',rowData.input_ratio+"%");
					normalTimeSum += parseFloat(rowData.normal_time);
					overTimeSum += parseFloat(rowData.overtime);
					specialTimeSum += parseFloat(rowData.special_time);
					inputRateSum += parseFloat(rowData.input_ratio);
				}
				$("#normalTimeSum").text(normalTimeSum);
				$("#overTimeSum").text(overTimeSum);
				$("#specialTimeSum").text(specialTimeSum);
				$("#inputRateSum").text(isNaN(parseFloat(inputRateSum/rows.length))?"0": (parseFloat(inputRateSum/rows.length).toFixed(1)));  //소수점 1자리 표현
			}
		});
