				...
				<tr>
					...
					<td>
						<c:choose>
							...
							<c:otherwise>
								 <input type="text" name="departmentList" style="width: 70%;" value=<c:choose><c:when test="${loginUserInfo.dept_code_list != null || loginUserInfo.dept_code_list != ''}">
								 																					'<c:forEach items="${loginUserInfo.dept_code_list }" var="item" varStatus="status">${item.dept_code }<c:if test="${!status.last }">,</c:if></c:forEach>'
								 																				</c:when>
								 																				<c:otherwise>'${loginUserInfo.dept_code}'</c:otherwise>
...
$(document).ready(function(){
	...
	$("#btn_search").click(function(){
		...
		$( '#dataList' ).jqGrid( 'setGridParam', {
			mtype : 'POST',
			url : '<c:url value="dpDepartmentDataMainGrid.do"/>',
			datatype : 'json',
			page : 1,
...
function fn_excelDownload(){
	...
	headerRow0.push({"name":"부서","row_to":2,"start_index":1,"end_index":1});
	headerRow0.push({"name":"투입인원","row_to":2,"start_index":2,"end_index":2});
	headerRow0.push({"name":"공사시수","row_to":0,"start_index":3,"end_index":18});
	headerRow0.push({"name":"비공사시수","row_to":1,"start_index":19,"end_index":22});
	headerRow0.push({"name":"총소요시수","row_to":2,"start_index":23,"end_index":23});
	headerRow0.push({"name":"근태시수","row_to":2,"start_index":24,"end_index":24});
	headerRow0.push({"name":"실투입인원","row_to":2,"start_index":25,"end_index":25});
	headerRow0.push({"name":"설계시수구분","row_to":1,"start_index":26,"end_index":28});
	
	var row0 = {"row":0,"rowvalue":headerRow0};
	
	var headerRow1 = [];
	headerRow1.push({"name":"도면시수","row_to":1,"start_index":3,"end_index":10});
	headerRow1.push({"name":"비도면시수","row_to":1,"start_index":11,"end_index":17});
	headerRow1.push({"name":"합계","row_to":2,"start_index":18,"end_index":18});
	var row1 = {"row":1,"rowvalue":headerRow1};
	
	var headerRow2 = [];
	headerRow2.push({"name":"설계제도","row_to":2,"start_index":3,"end_index":3});
	headerRow2.push({"name":"도면개정","row_to":2,"start_index":4,"end_index":4});
	headerRow2.push({"name":"도면검토","row_to":2,"start_index":5,"end_index":5});
	headerRow2.push({"name":"협의/검토","row_to":2,"start_index":6,"end_index":6});
	headerRow2.push({"name":"호선관리","row_to":2,"start_index":7,"end_index":7});
	headerRow2.push({"name":"직영계","row_to":2,"start_index":8,"end_index":8});
	headerRow2.push({"name":"외주도면시수","row_to":2,"start_index":9,"end_index":9});
	headerRow2.push({"name":"소계","row_to":2,"start_index":10,"end_index":10});
	headerRow2.push({"name":"설계준비","row_to":2,"start_index":11,"end_index":11});
	headerRow2.push({"name":"도면검토","row_to":2,"start_index":12,"end_index":12});
	headerRow2.push({"name":"PR/BOM","row_to":2,"start_index":13,"end_index":13});
	headerRow2.push({"name":"협의/검토","row_to":2,"start_index":14,"end_index":14});
	headerRow2.push({"name":"호선관리","row_to":2,"start_index":15,"end_index":15});
	headerRow2.push({"name":"기타","row_to":2,"start_index":16,"end_index":16});
	headerRow2.push({"name":"소계","row_to":2,"start_index":17,"end_index":17});
	headerRow2.push({"name":"보조지원","row_to":2,"start_index":19,"end_index":19});
	headerRow2.push({"name":"생산향상","row_to":2,"start_index":20,"end_index":20});
	headerRow2.push({"name":"기타","row_to":2,"start_index":21,"end_index":21});
	headerRow2.push({"name":"소계","row_to":2,"start_index":22,"end_index":22});
	headerRow2.push({"name":"정상시수","row_to":2,"start_index":26,"end_index":26});
	headerRow2.push({"name":"잔업시수","row_to":2,"start_index":27,"end_index":27});
	headerRow2.push({"name":"특근시수","row_to":2,"start_index":28,"end_index":28});
	...
	f.action = "dpDeptDataExcelExport.do";
	f.method = "post";
	f.submit();
...
function viewReport()
{
    ...
    urlStr += "stxPECDPDepartmentDataView_SP.mrd&param=";
    ...
    urlStr += application_form.dateSelected_from.value + ":::";
    urlStr += application_form.dateSelected_to.value + ":::";
    urlStr += application_form.departmentList.value + ":::";
    urlStr += application_form.factorCaseList.value + ":::";
    window.open(urlStr, "", "");    
}
	...
	$(document).ready( function() {
		...
		$( "#dataList" ).jqGrid( {
			...
			colNames : ['부서','투입인원',
			            ...
			            /**END**/
			            /**비도면시수**/
			            '설계준비','도면검토','PR/BOM','협의/검토','호선관리','기타','소계',
			            /**END**/
			            /**합계**/
			            '합계',
			            /**공사시수END**/
			...
			colModel : [
							...
							{name : 't_emp_cnt', index : 't_emp_cnt', width: 20, align : "center"},
							/**공사시수**/
							/**도면시수**/
							...
							{name : 'mh_dedwg_sum', index : 'mh_dedwg_sum', width: 20, align : "center"},
							/**END**/
							/**합계**/
							...
							{name : 'mh_decon_sum', index : 'mh_decon_sum', width: 20, align : "center"},
							
							{name : 'mh_take', index : 'mh_take', width: 20, align : "center"},
			...
			gridComplete : function() {
				...
				for(var i=0; i<rows.length; i++){
		            ...
		            if(i==0)
		            {
		            	...
		            	//$('#dataList').jqGrid('setRowData',rows[i],'',{background:trBgColor});
		...
		$( "#dataList" ).jqGrid( 'setGroupHeaders', {
			...
			groupHeaders : [ { startColumnName : 'mh_a1', numberOfColumns : 16,  titleText : fn_groupHeaderMultiTemplet('공사시수',['16','도면시수','8','비도면시수','7','','1'],$("#dataList_mh_a1").width()) }, 
			                 ...
			                 { startColumnName : 'normal_time', numberOfColumns : 3, titleText : '설계 시수구분' }]
		} );
