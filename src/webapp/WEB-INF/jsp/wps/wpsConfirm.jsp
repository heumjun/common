...
<form id="application_form" name="application_form" >
	...
	<input type="hidden" name="p_approve_flag" id="p_approve_flag"  value="A" />
	<div id="mainDiv" class="mainDiv">
		...
		<table class="searchArea conSearch">
			...
			<tr>
			<td>
				<input type="radio" checked="true" name="group" id="rb1" onclick="setGubun()" value="A">전체 &nbsp;&nbsp;
        		<input type="radio" name="group" id="rb2" onclick="setGubun()">승인 &nbsp;&nbsp;
        		<input type="radio" name="group" id="rb3" onclick="setGubun()">미승인
			</td>
			<th>Wps No</th>
			<td>
...
$(document).ready(function(){
	...
	jqGridObj.jqGrid({ 
	    ...
	    colNames : ['WPS_ID', 'Wps_No', 'Joint Type', 'Approval Date', 'Welding Process', 'Thick', 'Range', 'CONSUMABLE', 'REMARK', '<input type="checkbox" id="chkApprove_flag" onclick="checkBoxHeader(event, this)"/> 승인유무', 'enable_flag_changed', 'OPER'],
	    colModel : [
					...
					{name:'approve_flag', index:'approve_flag', width: 30, editable: true, sortable: false, edittype: 'checkbox', formatter: "checkbox", editoptions : { value : "Y:N" }, formatoptions : { disabled : false }, align : "center" },
					{name:'approve_flag_changed', index:'approve_flag_changed', width: 25, hidden:true},
...
function setGubun() {
    if (document.getElementById("rb1").checked) {
		$("#p_approve_flag").val("A");
    } else if( document.getElementById("rb2").checked ){
    	$("#p_approve_flag").val("Y");
    } else {
    	$("#p_approve_flag").val("N");
    }
}
...
function fn_save() {
	...
	for( var i = 1; i < changedData.length + 1; i++ ) {
		...
		if ( item.oper != 'I' && item.oper != 'U' ) {
			...
			if (item.approve_flag_changed != item.approve_flag) {
				item.oper = 'U';
			}
	...
	if ( confirm( '변경된 데이터를 저장하시겠습니까?' ) != 0 ) {
		...
		getChangedChmResultData( function( data ) {
			...
			var url = 'saveWpsConfirmAction.do';
