...
//		sSource.getBytes()
	...
	/**
	 * 원본 문자열을 매치되는 문자(열)을 기준으로 잘라서 String[] 형태로 리턴한다.
	 * StringTokenizer와 다른 점은 공백에 해당하는 값도 포함된다는 것이며, JDK 1.4 이상의 String.split()에 해당된다고 보면 된다. 
	 * @param originalStr 처리할 원본 문자열
	 * @param matchStr 문자열을 자르는 기준(비교) 문자(열)
	 * @return String[]
	 */
	public static String[] split(String originalStr, String matchStr) {
		if (originalStr == null) return null;
		if (originalStr.trim().equals("")) return new String[] {originalStr};
		
		ArrayList resultList = new ArrayList();
		String originalStrBackup = originalStr;
		
		while (originalStr.indexOf(matchStr) >= 0) {
			resultList.add(originalStr.substring(0, originalStr.indexOf(matchStr)));
			originalStr = originalStr.substring(originalStr.indexOf(matchStr) + matchStr.length());
		}
		if (originalStr != null && !originalStr.equals("")) resultList.add(originalStr);
		if (originalStrBackup.endsWith(matchStr)) resultList.add("");
		
		String[] resultArray = new String[resultList.size()];
		for (int i = 0; i < resultList.size(); i++) {
			resultArray[i] = (String)resultList.get(i);
		}		
	...
	/**
	 * String.replace()의 확장으로 문자열 내에 일치하는 모든 항목을 변경한다.
	 * JDK 1.4 이상의 String.replaceAll()에 해당된다고 보면 된다.
	 * @param originalStr 처리할 원본 문자열
	 * @param fromStr 변경할 기존 문자(열)
	 * @param toStr 변경할 대상 문자(열)
	 * @return String
	 */
	public static String replaceAll(String originalStr, String fromStr, String toStr) {
		if (originalStr == null) return null;
		...
		while (resultStr.indexOf(fromStr) >= 0) {
			resultStr = resultStr.substring(0, resultStr.indexOf(fromStr)) 
			            + toStr 
			            + resultStr.substring(resultStr.indexOf(fromStr) + fromStr.length());
		}
